...
import { getPosts, favoriteCollective, unfavoriteCollective } from '../services/api/social';
...
export default function NewGivingGroupPage() {
    ...
    const deleteCollectiveSheetRef = useRef<BottomSheetModal>(null);
    ...
    const [showDetailsSheet, setShowDetailsSheet] = useState(true);
    ...
    const deleteCollectiveMutation = useMutation({
        ...
        onError: (error: any) => {
            ...
            showToast('Failed to delete Giving Group. Please try again.', 3000);
        },
    });

    const handleDeleteCollective = () => {
        deleteCollectiveSheetRef.current?.present();
    };
    ...
    const toggleFavoriteMutation = useMutation({
        mutationFn: () => {
            if (crwdData?.is_favorite) {
                return unfavoriteCollective(crwdId);
            }
            return favoriteCollective(crwdId);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['crwd', crwdId] });
        },
        onError: (error: any) => {
            ...
            showToast('Failed to update favorite. Please try again.', 3000);
        },
    });
    ...
    const handleToggleCause = async (causeId: number, isAdding: boolean) => {
        if (!currentUser?.id || !token?.access_token) {
            showToast('Please log in to manage your donation box', 3000);
            return;
        }
        ...
        try {
            ...
            if (isAdding) {
                ...
                if (donationBoxData?.id && currentCount >= capacity) {
                    showToast('Donation box is at capacity', 3000);
                    return;
                }
                ...
                await addCausesToBox({
                    causes: [{
                        cause_id: causeId,
                        attributed_collective: parseInt(crwdId)
                    }]
                });
                showToast('Nonprofit added to your donation box', 2000);
            } else {
                await removeCauseFromBox(String(causeId));
                showToast('Nonprofit removed from your donation box', 2000);
            }
            queryClient.invalidateQueries({ queryKey: ['donationBox'] });
            refetchDonationBox();
        } catch (error) {
            console.error('Error toggling cause:', error);
            showToast(`Failed to ${isAdding ? 'add' : 'remove'} nonprofit`, 3000);
        } finally {
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            <GivingGroupHeader
                ...
                onMore={() => setShowDetailsSheet(true)}
            />
                ...
                <View style={styles.content}>
                    ...
                    <CommunityActivity
                        posts={posts?.results ? posts.results.map((post: any) => ({
                            ...
                            username: post.user?.full_name || (post.user?.first_name && post.user?.last_name
            ...
            >
                <BottomSheetView style={styles.removeModalBody}>
                    <Text style={styles.removeModalTitle}>Delete Giving Group</Text>
                    <Text style={styles.removeModalDescription}>
                        Are you sure you want to delete <Text style={styles.removeModalBold}>{crwdData?.name}</Text>? This action cannot be undone.
                    </Text>

                    <View style={styles.removeModalFooter}>
                        <TouchableOpacity
                            onPress={() => deleteCollectiveSheetRef.current?.dismiss()}
                            style={[styles.removeModalButton, styles.removeModalCancelButton]}
                            activeOpacity={0.7}
                        ...
                        >
                            {deleteCollectiveMutation.isPending ? (
                                ...
                                <Text style={styles.removeModalConfirmText}>Delete Group</Text>
                            )}
                        </TouchableOpacity>
                    </View>
                </BottomSheetView>
            ...
            <JoinGroupBottomSheet
                ...
                collectiveName={crwdData.name || 'Giving Group'}
            ...
            <GivingGroupDetailsBottomSheet
                ...
                groupData={{
                    id: crwdData.id || crwdId,
                    name: crwdData.name || 'Giving Group',
                    founderName: founderName || 'Organizer',
                    memberCount: memberCount,
                    donationCount: donationCount,
                    nonprofitCount: nonprofitCount,
                    description: crwdData.description || '',
                ...
                onNotifications={() => {
                    setShowDetailsSheet(false);
                    showToast("Notification settings coming soon!", 2000);
                }}
                onJoin={() => {
                    setShowDetailsSheet(false);
            ...
            {crwdData?.is_joined && (
                <View style={[styles.fixedBottomBar, { paddingBottom: 34 }]}>
                    <TouchableOpacity
                        onPress={() => createPostSheetRef.current?.present()}
                        style={styles.threadBar}
                        activeOpacity={0.8}
                    >
                        <Text style={styles.threadText}>Start a thread...</Text>
                        <View style={styles.plusButton}>
                            <Plus size={22} color="white" strokeWidth={3} {...({} as any)} />
                        </View>
                    </TouchableOpacity>
                </View>
            )}
            ...
            <CreatePostBottomSheet
                ...
                isOpen={true} // Controlled by ref.present()
                onClose={() => createPostSheetRef.current?.dismiss()}
                collectiveData={crwdData}
            />
        </SafeAreaView>
    );
